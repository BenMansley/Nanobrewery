    ...
    return (
      <div className="editable-slider">
        ...
        <form onSubmit={(event) => this.onSubmit(event)} className={active ? "active" : ""}>
          ...
          <MaterialInput type="number" id="min" labelText="Minimum *" value={min}
            onChange={(event) => this.setState({ min: event.target.value })} />
          <MaterialInput type="number" id="max" labelText="Maximum *" value={max}
            onChange={(event) => this.setState({ max: event.target.value })} />
          <MaterialInput type="number" id="step" labelText="Increment *" value={step}
            onChange={(event) => this.setState({ step: event.target.value })} />
          <MaterialInput type="number" id="defaultVal" labelText="Default *" value={defaultVal}
            onChange={(event) => this.onDefaultChange(event.target.value)} />
          <MaterialInput type="text" id="suffix" labelText="Suffix (after %)" value={suffix}
